...
string normalizeProgramCode(string program)
{
    ...
    {
        program = regex_replace(program, spacesRegEx,
                                "$1"); // $n - ссылка на n-ую скобочную группу. Т.е. здесь меняем на (\W) если она есть
    }
...
{
    string errorMessage = "Error: " + (index < tokens.size() ? "incorrect token near " + tokens[index].get_value() +
...
{
    if (recognizeExpectedToken(tokens, index, tokenType::plus) |
        recognizeExpectedToken(tokens, index, tokenType::minus) |
...
pair<string, size_t> rule_b(const vector<token> &tokens, size_t index)
{
    ...
    if (!recognizeExpectedToken(tokens, index, tokenType::equal) && !recognizeExpectedToken(tokens, index, not_equal) &&
        !recognizeExpectedToken(tokens, index, tokenType::greater) &&
        !recognizeExpectedToken(tokens, index, tokenType::less) &&
        !recognizeExpectedToken(tokens, index, greater_or_equal) &&
        !recognizeExpectedToken(tokens, index, less_or_equal))
    {
